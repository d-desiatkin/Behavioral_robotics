...
Evonet::Evonet(int nnetworks, int heterogeneous, int ninputs, int nhiddens, int noutputs, int nlayers, int nhiddens2, int bias, int netType, int actFunct, int outType, int wInit, int clip, int normalize, int randAct, double randActR, double wrange, int nbins, double low, double high)
{
    ...
    Dmitrii_ptr_cobs_prev = new float*[nnetworks];
    for(int i = 0; i < nnetworks; i++){
        Dmitrii_start_learning[i] = true;
        Dmitrii_ptr_cact_prev[i] = new float[noutputs];
        Dmitrii_ptr_cobs_prev[i] = new float[ninputs];
    }
...
//printf(" -> %.2f %.2f \n", m_nstate[tt], *ni);
            ...
            if (*nbl == 1)
            {   
                bool Dmitrii_hide_all = false;
                if (netRng->getInt(1, 100) <= 20){
                    Dmitrii_hide_all = true;
                }
                    ...
                    switch (*nt)
                    {
                        case 0:
                            ...
                            if (Dmitrii_start_learning[n]){
                                ...
                                *a = *(cobserv + t);
                                *(Dmitrii_ptr_cobs_prev[n] + t) = *a;
                            }
                            else{
                                if (Dmitrii_hide_all){
                                    *a = *(Dmitrii_ptr_cobs_prev[n] + t) + *(Dmitrii_ptr_cact_prev[n] + m_noutputs - m_ninputs + t);
                                    *(Dmitrii_ptr_cobs_prev[n] + t) = *a;
                                }
                                else{
        ...
        if (m_nbins == 1)
        {
            int i;
            // outfile << "\n Output: \n";
            for (i = 0; i < m_noutputs - m_ninputs; i++)
            {
               switch(m_randAct)
               {
                  // standard no-noise
        ...
        else // with bins
        {
            ...
            for (i = 0; i < m_noutputsbins - m_ninputs; i++)
            {
